...
## Quality Standards & Double-Lock Consents (Tier 3 Governance)

**Double-Lock Developer Consent:**
- Before dynamic brainstorming or file scaffolding begins, the agent MUST verify that the user has explicitly authorized this execution by providing the `--manual-auth` flag or by approving the skill creation in chat.
- If consent or the flag is missing, the agent MUST immediately halt and prompt the user in chat: *"I need your explicit confirmation to create a custom skill. Please confirm your approval in chat or run with the --manual-auth flag."*
