...
## Unified Audit Log Writeback Hooks (Tier 4 Governance)

- **Cryptographic Audit Log:** Whenever a Tier 3 (High Risk) or Tier 4 (Critical Risk) command execution is triggered or permitted by the security gateway, the auditor MUST append a structured, cryptographic entry to the workspace's local log file: **`.memory/audit-log.md`**.
- **Audit Format:**
  ```markdown
  ### [AUDIT] - <DATE> - <TIME>
  - **Tool Name:** <tool_name>
