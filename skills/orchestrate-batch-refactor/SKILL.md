...
## Safety Guardrails & Double-Lock Consents (Tier 3 Governance)

- **Double-Lock Developer Consent:** Before starting plan synthesis or spawning parallel worker agents, the agent MUST verify that the user has explicitly authorized the run by providing the `--yes` CLI flag or by approving the worker plan in chat.
- **Action:** If consent or the `--yes` flag is missing, the agent MUST immediately halt and display the proposed worker plan to the developer, prompting them in chat: *"I need your explicit confirmation to spawn parallel workers. Please confirm your approval in chat or provide the --yes flag."*
